            ...
            <div style={{ fontFamily: 'var(--fontRoboto)' }}>
                <div className='grid w-full'>
                    <div className='text-center col-12 md:col-2 hidden md:block'>
                        <Avatar image={'https://www.nhakhoaquocteachau.vn/' + items?.image_news[0]?.image} className="mr-2 w-11rem h-11rem" size="xlarge" shape="circle" />
                        <div className='font-bold'>by {items?.news_author['fullname']}
                        </div>
                        <span className='text-xl text-500'>{items?.created_at}</span>
                    </div>
                    <div className='col-12 md:col-10'>
                        <img className='cs-img-news' width='100%' src={items?.image_news.map((e) => { return `https://www.nhakhoaquocteachau.vn/${e.image}` })} />
                ...
                <div className='imageBanner'>
                    <div className='cell px-6'>
                        <div className='cellNews w-default'>
                            <div className='pb-5 text-3xl'>NHA KHOA Á Châu</div>
                            <div className='pb-3 text-7xl'>Tin Tức</div>
                            <hr className='hr mb-4' />
                            <div>Nha khoa Á Châu là một trong những nha khoa lớn và uy tín nhất trên cả nước hiện nay.  Bên cạnh đội ngũ chuyên gia nha khoa hàng đầu, giàu kinh nghiệm thì trang thiết bị y tế, ứng dụng các công nghệ tiên tiến bậc nhất trên thế giới. </div>
                        </div>
                    </div>
                </div>
